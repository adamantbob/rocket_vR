...
use rocket_core::utilization::ExecutorMetrics;
#[cfg(feature = "verbose-utilization")]
...
pub async fn stats_task(
    ...
    let metrics1 = if dual_core {
        &rocket_core::utilization::METRICS_CORE1
    } else {
        &ExecutorMetrics::new()
    };
    ...
    loop {
        ...
        let idle1 = if dual_core {
            metrics1.idle_ticks.load(Ordering::Relaxed)
        } else {
            0
        };
        ...
        let cpu_health = if dual_core {
            CPUHealth::new_from_readings(
                usage0_total,
                usage0_high_prio,
                usage0_background,
                usage1_total,
        ...
        if usage0_total > rocket_core::utilization::TOTAL_USAGE_ALARM_THRESHOLD
            || (dual_core && usage1_total > rocket_core::utilization::TOTAL_USAGE_ALARM_THRESHOLD)
        {
            error!(
        ...
        if dual_core && usage1_stack_hwm > rocket_core::utilization::STACK_USAGE_ALARM_THRESHOLD {
        ...
        #[cfg(feature = "verbose-utilization")]
        {
            ...
            let poll1 = if dual_core {
                metrics1.poll_count.swap(0, Ordering::Relaxed)
            } else {
                0
            };
            let interrupt_polls =
