...
use rocket_core::{error, local_info};
...
pub async fn radio_task_driver(
    ...
    let mut radio = Rfm95::new(spi_dev, reset, interrupt, lora_config).await;
        ...
        match radio.init().await {
            Ok(_) => {
                ...
                loop {
                    ...
                    let cpu_utilization =
                        SYSTEM_HEALTH.cpu_health.read().usage_c0.ceil_percent_u8()
            ...
            Err(e) => {
                error!("Failed to initialize radio: {:?}", e);
